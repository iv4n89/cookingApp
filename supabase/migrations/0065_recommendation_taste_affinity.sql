-- Añade tasteAffinity (máx similitud coseno de una receta contra las recetas que el hogar marcó
-- favoritas o valoró >=4) al snapshot y al motor de decisión. Entra como desempate bajo en el
-- ORDER BY de evaluate_recommendation_snapshot, después de availabilityRatio: no altera el
-- criterio pantry-first (missingIngredientCount / priorityUsage / consumeSoonUsage / seasonalAffinity /
-- mealTypeMatch / availabilityRatio siguen mandando). Cold start (hogar sin semillas) = afinidad 0
-- para todas las recetas, así que el orden no cambia respecto a hoy.
